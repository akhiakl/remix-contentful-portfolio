...
const Button: React.FC<IconButtonProps> = ({ children, href, icon, variant = 'primary', ...props }) => {
    const containerClass = clsx(
        ...
        {
            "gap-3 bg-primary hover:bg-primary/80 text-gray-950 rounded-full duration-100 ease-out": variant !== 'link',
    ...
    const iconMarkup = icon && (
        <span className={clsx("flex justify-center items-center", {
            ...
            <span className={clsx('w-full h-full', {
                ' bg-gray-950 text-white p-2 md:p-0 md:w-3 md:h-3 group-hover:p-2 group-hover:w-full group-hover:h-full rounded-full duration-100 ease-in': variant !== 'link',
